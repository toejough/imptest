...
// StartCalculatorWrapperHandle wraps an implementation of calculator.Calculator to intercept method calls.
type StartCalculatorWrapperHandle struct {
	...
	Process  *StartCalculatorWrapperProcessWrapper
...
// StartCalculatorWrapperProcessCallHandle represents a single call to the wrapped method.
type StartCalculatorWrapperProcessCallHandle struct {
	*_imptest.CallableController[StartCalculatorWrapperProcessReturns]
}

// ExpectCompletes verifies the method completes without panicking.
func (h *StartCalculatorWrapperProcessCallHandle) ExpectCompletes() {
	h.T.Helper()
	h.WaitForResponse()
...
// ExpectPanic verifies the method panics with the expected value.
func (h *StartCalculatorWrapperProcessCallHandle) ExpectPanic(expected any) {
	h.T.Helper()
	h.WaitForResponse()
...
// ExpectPanicMatch verifies the method panics with a value matching the given matcher.
func (h *StartCalculatorWrapperProcessCallHandle) ExpectPanicMatch(matcher any) {
	h.T.Helper()
	h.WaitForResponse()
...
// ExpectReturn verifies the method returned the expected values.
func (h *StartCalculatorWrapperProcessCallHandle) ExpectReturn(v0 string, v1 error) {
	h.T.Helper()
	h.WaitForResponse()
...
// ExpectReturnMatch verifies the return values match the given matchers.
func (h *StartCalculatorWrapperProcessCallHandle) ExpectReturnMatch(v0 any, v1 any) {
	h.T.Helper()
	h.WaitForResponse()
...
type StartCalculatorWrapperProcessReturns struct {
	Result0 string
	Result1 error
}

type StartCalculatorWrapperProcessWrapper struct {
	t  *testing.T
	fn func(input int) StartCalculatorWrapperProcessReturns
}

// Start executes the wrapped method in a goroutine.
func (w *StartCalculatorWrapperProcessWrapper) Start(input int) *StartCalculatorWrapperProcessCallHandle {
	handle := &StartCalculatorWrapperProcessCallHandle{
		CallableController: _imptest.NewCallableController[StartCalculatorWrapperProcessReturns](w.t),
	}
	go func() {
		defer func() {
			if r := recover(); r != nil {
...
// StartCalculator creates a new wrapper for the given calculator.Calculator implementation.
func StartCalculator(t *testing.T, impl *calculator.Calculator) *StartCalculatorWrapperHandle {
	...
	h.Process = wrapStartCalculatorWrapperProcess(t, func(input int) StartCalculatorWrapperProcessReturns {
		r0, r1 := h.impl.Process(input)
		return StartCalculatorWrapperProcessReturns{Result0: r0, Result1: r1}
	})
	return h
}
...
func wrapStartCalculatorWrapperProcess(t *testing.T, fn func(input int) StartCalculatorWrapperProcessReturns) *StartCalculatorWrapperProcessWrapper {
	return &StartCalculatorWrapperProcessWrapper{t: t, fn: fn}
}
