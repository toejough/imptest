...
// StartProcessFuncCallHandle represents a single call to the wrapped function.
type StartProcessFuncCallHandle struct {
	*_imptest.CallableController[StartProcessFuncReturnsReturn]
...
// StartProcessFuncCallHandleEventually wraps a call handle for async expectation registration.
type StartProcessFuncCallHandleEventually struct {
...
func (e *StartProcessFuncCallHandleEventually) ensureStarted() *_imptest.PendingCompletion {
...
// StartProcessFuncReturnsReturn holds the return values from the wrapped function.
type StartProcessFuncReturnsReturn struct {
	Result0 string
	Result1 error
}

// StartProcessFunc starts the wrapped function in a goroutine for testing.
func StartProcessFunc(t _imptest.TestReporter, fn func(int) (string, error), x int) *StartProcessFuncCallHandle {
	handle := &StartProcessFuncCallHandle{
		CallableController: _imptest.NewCallableController[StartProcessFuncReturnsReturn](t),
		controller:         _imptest.NewTargetController(t),
	}
	handle.Eventually = &StartProcessFuncCallHandleEventually{h: handle}
	go func() {
		defer func() {
			if r := recover(); r != nil {
				handle.PanicChan <- r
		...
		ret0, ret1 := fn(x)
		handle.ReturnChan <- StartProcessFuncReturnsReturn{Result0: ret0, Result1: ret1}
	}()
	return handle
}
