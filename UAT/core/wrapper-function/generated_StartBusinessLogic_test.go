...
func (e *StartBusinessLogicCallHandleEventually) ensureStarted() *_imptest.PendingCompletion {
...
// StartBusinessLogic starts the wrapped function in a goroutine for testing.
func StartBusinessLogic(t _imptest.TestReporter, fn func(callable.ExternalService, int) (string, error), svc callable.ExternalService, id int) *StartBusinessLogicCallHandle {
	handle := &StartBusinessLogicCallHandle{
		CallableController: _imptest.NewCallableController[StartBusinessLogicReturnsReturn](t),
		controller:         _imptest.NewTargetController(t),
	}
	handle.Eventually = &StartBusinessLogicCallHandleEventually{h: handle}
	go func() {
		defer func() {
			if r := recover(); r != nil {
				handle.PanicChan <- r
		...
		ret0, ret1 := fn(svc, id)
		handle.ReturnChan <- StartBusinessLogicReturnsReturn{Result0: ret0, Result1: ret1}
	}()
	return handle
}
