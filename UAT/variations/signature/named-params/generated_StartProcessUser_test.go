...
type StartProcessUserCallHandle struct {
	*_imptest.CallableController[StartProcessUserReturnsReturn]
	...
	Eventually *StartProcessUserCallHandleEventually
}

// ExpectPanic verifies the function panics with the expected value.
func (h *StartProcessUserCallHandle) ExpectPanic(expected any) {
	h.T.Helper()
	h.WaitForResponse()
...
func (h *StartProcessUserCallHandle) ExpectPanicMatch(matcher any) {
	h.T.Helper()
	h.WaitForResponse()
...
func (h *StartProcessUserCallHandle) ExpectReturn(v0 named.User, v1 error) {
	h.T.Helper()
	h.WaitForResponse()
...
func (h *StartProcessUserCallHandle) ExpectReturnMatch(v0 any, v1 any) {
	h.T.Helper()
	h.WaitForResponse()
...
func (e *StartProcessUserCallHandleEventually) ExpectPanic(value any) {
	e.ensureStarted().ExpectPanic(value)
}

// ExpectReturn registers an async expectation for return values.
func (e *StartProcessUserCallHandleEventually) ExpectReturn(values ...any) {
	e.ensureStarted().ExpectReturn(values...)
}

func (e *StartProcessUserCallHandleEventually) ensureStarted() *_imptest.PendingCompletion {
...
// StartProcessUser starts the wrapped function in a goroutine for testing.
func StartProcessUser(t _imptest.TestReporter, fn func(context.Context, int, named.UserRepository) (named.User, error), ctx context.Context, userID int, repo named.UserRepository) *StartProcessUserCallHandle {
	handle := &StartProcessUserCallHandle{
		CallableController: _imptest.NewCallableController[StartProcessUserReturnsReturn](t),
		controller:         _imptest.NewTargetController(t),
	}
	handle.Eventually = &StartProcessUserCallHandleEventually{h: handle}
	go func() {
		defer func() {
			if r := recover(); r != nil {
				handle.PanicChan <- r
		...
		ret0, ret1 := fn(ctx, userID, repo)
		handle.ReturnChan <- StartProcessUserReturnsReturn{Result0: ret0, Result1: ret1}
	}()
	return handle
}
