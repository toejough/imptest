...
type StartCountFilesCallHandle struct {
	*_imptest.CallableController[StartCountFilesReturnsReturn]
	...
	Eventually *StartCountFilesCallHandleEventually
}

// ExpectPanic verifies the function panics with the expected value.
func (h *StartCountFilesCallHandle) ExpectPanic(expected any) {
	h.T.Helper()
	h.WaitForResponse()
...
func (h *StartCountFilesCallHandle) ExpectPanicMatch(matcher any) {
	h.T.Helper()
	h.WaitForResponse()
...
func (h *StartCountFilesCallHandle) ExpectReturn(v0 int, v1 error) {
	h.T.Helper()
	h.WaitForResponse()
...
func (h *StartCountFilesCallHandle) ExpectReturnMatch(v0 any, v1 any) {
	h.T.Helper()
	h.WaitForResponse()
...
func (e *StartCountFilesCallHandleEventually) ExpectPanic(value any) {
	e.ensureStarted().ExpectPanic(value)
}

// ExpectReturn registers an async expectation for return values.
func (e *StartCountFilesCallHandleEventually) ExpectReturn(values ...any) {
	e.ensureStarted().ExpectReturn(values...)
}

func (e *StartCountFilesCallHandleEventually) ensureStarted() *_imptest.PendingCompletion {
...
// StartCountFiles starts the wrapped function in a goroutine for testing.
func StartCountFiles(t _imptest.TestReporter, fn func(visitor.TreeWalker, string) (int, error), walker visitor.TreeWalker, root string) *StartCountFilesCallHandle {
	handle := &StartCountFilesCallHandle{
		CallableController: _imptest.NewCallableController[StartCountFilesReturnsReturn](t),
		controller:         _imptest.NewTargetController(t),
	}
	handle.Eventually = &StartCountFilesCallHandleEventually{h: handle}
	go func() {
		defer func() {
			if r := recover(); r != nil {
				handle.PanicChan <- r
		...
		ret0, ret1 := fn(walker, root)
		handle.ReturnChan <- StartCountFilesReturnsReturn{Result0: ret0, Result1: ret1}
	}()
	return handle
}
