...
// CalculatorMultiplyImp wraps a callable function for testing.
// Create with NewCalculatorMultiplyImp(t, yourFunction), call Start() to execute,
// then use ExpectReturnedValuesAre/Should() or ExpectPanicWith() to verify behavior.
type CalculatorMultiplyImp struct {
	*imptest.CallableController[CalculatorMultiplyImpReturn]
	callable func(value int) int
}

// NewCalculatorMultiplyImp creates a new wrapper for testing the callable function.
// Pass the function to test and a testing.TB to enable assertion failures.
//
// Example:
//
//	wrapper := NewCalculatorMultiplyImp(t, myFunction)
//	wrapper.Start(args...).ExpectReturnedValuesAre(expectedVals...)
func NewCalculatorMultiplyImp(t testing.TB, callable func(value int) int) *CalculatorMultiplyImp {
	return &CalculatorMultiplyImp{
		CallableController: imptest.NewCallableController[CalculatorMultiplyImpReturn](t),
		callable:           callable,
	}
}
...
func (s *CalculatorMultiplyImp) ExpectPanicWith(expected any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *CalculatorMultiplyImp) ExpectReturnedValuesAre(v1 int) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *CalculatorMultiplyImp) ExpectReturnedValuesShould(v1 any) {
	s.T.Helper()
	s.WaitForResponse()
...
func (s *CalculatorMultiplyImp) Start(value int) *CalculatorMultiplyImp {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				s.PanicChan <- r
		...
		s.ReturnChan <- CalculatorMultiplyImpReturn{
			Result0: ret0,
		}
	}()
	return s
...
// CalculatorMultiplyImpResponse represents the response from the callable (either return or panic).
...
type CalculatorMultiplyImpResponse struct {
	EventType string // "return" or "panic"
	ReturnVal *CalculatorMultiplyImpReturn
	PanicVal  any
}

// Type returns the event type: "return" for normal returns, "panic" for panics.
func (r *CalculatorMultiplyImpResponse) Type() string {
	return r.EventType
}

// CalculatorMultiplyImpReturn holds the return values from the callable function.
// Access individual return values via Result0, Result1, etc. fields.
type CalculatorMultiplyImpReturn struct {
	Result0 int
}
