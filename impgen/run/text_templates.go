...
func NewTemplateRegistry() (*TemplateRegistry, error) {
	...
	registry.headerTmpl, err = template.New("header").Parse(`// Code generated by impgen. DO NOT EDIT.
...
{{end}}`)
	...
	//nolint:lll,nolintlint
	registry.constructorTmpl, err = template.New("constructor").Parse(`// New{{.ImpName}} creates a new test controller for mocking the interface.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse constructor template: %w", err)
	}

	// ExpectCallIsStruct template
	//nolint:lll // Template string length driven by generated code structure
	//nolint:lll,nolintlint
	registry.expectCallIsStructTmpl, err = template.New("expectCallIsStruct").Parse(`// {{.ExpectCallIsName}} provides methods to set expectations for specific method calls.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse expectCallIsStruct template: %w", err)
	}

	// GetCurrentCallMethod template
	//nolint:lll // Template definition
	//nolint:lll,nolintlint
	registry.getCurrentCallMethodTmpl, err = template.New("getCurrentCallMethod").Parse(`// GetCurrentCall returns the current call being processed.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse getCurrentCallMethod template: %w", err)
	}

	// InterfaceVerification template
	//nolint:lll,nolintlint
	registry.interfaceVerificationTmpl, err = template.New("interfaceVerification").Parse(`var (
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse interfaceVerification template: %w", err)
	}

	// MainStruct template
	//nolint:lll,nolintlint
	registry.mainStructTmpl, err = template.New("mainStruct").Parse(`// {{.ImpName}} is the test controller for mocking the interface.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse mainStruct template: %w", err)
	}

	// MockStruct template
	//nolint:lll,nolintlint
	registry.mockStructTmpl, err = template.New("mockStruct").Parse(`// {{.MockName}} provides the mock implementation of the interface.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse mockStruct template: %w", err)
	}

	// TimedStruct template
	//nolint:lll,nolintlint
	registry.timedStructTmpl, err = template.New("timedStruct").Parse(`// {{.TimedName}} provides timeout-configured expectation methods.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse timedStruct template: %w", err)
	}

	// InjectPanic template
	//nolint:lll,nolintlint
	registry.injectPanicTmpl, err = template.New("injectPanic").Parse(`// InjectPanic causes the mocked method to panic with the given value.
...
func (c *{{.MethodCallName}}{{.TypeParamsUse}}) InjectPanic(msg any) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse injectPanic template: %w", err)
	}

	// Resolve template
	//nolint:lll,nolintlint
	registry.resolveTmpl, err = template.New("resolve").Parse(`// Resolve completes a void method call without error.
...
func (c *{{.MethodCallName}}{{.TypeParamsUse}}) Resolve() {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse resolve template: %w", err)
	}

	// CallStruct template
	//nolint:lll,nolintlint
	registry.callStructTmpl, err = template.New("callStruct").Parse(`// {{.CallName}} represents a captured call to any method.
...
{{end}}`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callStruct template: %w", err)
	}

	// CallableHeader template
	//nolint:lll,nolintlint
	registry.callableHeaderTmpl, err = template.New("callableHeader").Parse(`// Code generated by impgen. DO NOT EDIT.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableHeader template: %w", err)
	}

	// CallableReturnStruct template
	//nolint:lll,nolintlint
	registry.callableReturnStructTmpl, err = template.New("callableReturnStruct").Parse(`{{if .HasReturns}}// {{.ImpName}}Return holds the return values from the callable function.
...
{{end}}`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableReturnStruct template: %w", err)
	}

	// CallableMainStruct template
	//nolint:lll,nolintlint
	registry.callableMainStructTmpl, err = template.New("callableMainStruct").Parse(`// {{.ImpName}} wraps a callable function for testing.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableMainStruct template: %w", err)
	}

	// CallableConstructor template
	//nolint:lll,nolintlint
	registry.callableConstructorTmpl, err = template.New("callableConstructor").Parse(`// New{{.ImpName}} creates a new wrapper for testing the callable function.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableConstructor template: %w", err)
	}

	// CallableStartMethod template
	//nolint:lll,nolintlint
	registry.callableStartMethodTmpl, err = template.New("callableStartMethod").Parse(`// Start begins execution of the callable in a goroutine with the provided arguments.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableStartMethod template: %w", err)
	}

	// CallableExpectReturnedValuesAre template
	//nolint:lll,nolintlint
	registry.callableExpectReturnedValuesAreTmpl, err = template.New("callableExpectReturnedValuesAre").Parse(`// ExpectReturnedValuesAre asserts the callable returned with exactly the specified values.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableExpectReturnedValuesAre template: %w", err)
	}

	// CallableExpectReturnedValuesShould template
	//nolint:lll,nolintlint
	registry.callableExpectReturnedValuesShouldTmpl, err = template.New("callableExpectReturnedValuesShould").Parse(`// ExpectReturnedValuesShould asserts return values match the given matchers.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableExpectReturnedValuesShould template: %w", err)
	}

	// CallableExpectPanicWith template
	//nolint:lll,nolintlint
	registry.callableExpectPanicWithTmpl, err = template.New("callableExpectPanicWith").Parse(`// ExpectPanicWith asserts the callable panicked with a value matching the expectation.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableExpectPanicWith template: %w", err)
	}

	// CallableResponseStruct template
	//nolint:lll,nolintlint
	registry.callableResponseStructTmpl, err = template.New("callableResponseStruct").Parse(`// {{.ImpName}}Response represents the response from the callable (either return or panic).
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableResponseStruct template: %w", err)
	}

	// CallableResponseTypeMethod template
	//nolint:lll,nolintlint
	registry.callableResponseTypeMethodTmpl, err = template.New("callableResponseTypeMethod").Parse(`// Type returns the event type: "return" for normal returns, "panic" for panics.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableResponseTypeMethod template: %w", err)
	}

	// CallableAsReturnMethod template
	//nolint:lll,nolintlint
	registry.callableAsReturnMethodTmpl, err = template.New("callableAsReturnMethod").Parse(`// AsReturn converts the return values to a slice of any for generic processing.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableAsReturnMethod template: %w", err)
	}

	// CallableGetResponseMethod template
	//nolint:lll,nolintlint
	registry.callableGetResponseMethodTmpl, err = template.New("callableGetResponseMethod").Parse(`// GetResponse waits for and returns the callable's response.
...
`)
	if err != nil {
		return nil, fmt.Errorf("failed to parse callableGetResponseMethod template: %w", err)
	}
