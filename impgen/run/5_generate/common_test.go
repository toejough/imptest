...
func TestIsBuiltinType(t *testing.T) {
	...
	tests := []struct {
		...
		// Builtin types - should return true
		{"bool", true},
		{"byte", true},
		{"complex64", true},
		{"complex128", true},
		...
		{"comparable", true},
		{"any", true},
		// Non-builtin types - should return false
		{"MyType", false},
		{"context.Context", false},
		{"", false},
		{"String", false}, // capitalized string is not builtin
	}
...
func TestIsExportedIdent(t *testing.T) {
	...
	tests := []struct {
		...
		{"", false},           // Empty string
		{"a", false},          // Lowercase
		{"myFunc", false},     // Lowercase
		{"A", true},           // Uppercase single
		{"MyFunc", true},      // Uppercase start
		{"HTTPHandler", true}, // Uppercase acronym
	}
	...
	for _, tt := range tests { //nolint:varnamelen // tt is idiomatic in Go tests
		t.Run(tt.name, func(t *testing.T) {
			...
			got := isExportedIdent(tt.name)
			if got != tt.want {
				t.Errorf("isExportedIdent(%q) = %v, want %v", tt.name, got, tt.want)
			}
		})
	}
}
